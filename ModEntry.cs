...
#nullable enable
        ...
        public static Texture2D? TreeTexture(CWildTreeData data)
        {
            if (data.Textures is null)
            {
                return null;
            }
            foreach (CWildTreeData.TextureData data1 in data.Textures)
            {
    ...
    public static class BiggerTree
    {
        public static bool Prefix(Tree __instance, SpriteBatch spriteBatch, List<Leaf> ___leaves)
        {
            ...
            Texture2D? texture = ModEntry.TreeTexture(treeData);
            Vector2 tileLocation = __instance.Tile;
            float baseSortPosition = __instance.getBoundingBox().Bottom;
            if (treeData.Textures == null || texture == null)
            {
                ModEntry.instance.Monitor.LogOnce($"There are issues with BiggerTree Texture for {__instance.treeType.Value}. Skipping", LogLevel.Error);
                return true;
            }
            else if (__instance.isTemporarilyInvisible || __instance.texture.Value == null || !Tree.TryGetData(__instance.treeType.Value, out var data) || __instance.growthStage.Value < 5)
            {
                return true;
            }
            else
    ...
    public static class BiggerFruitTree
    {
        public static bool Prefix(FruitTree __instance, SpriteBatch spriteBatch, List<Leaf> ___leaves)
        {
            ...
            Texture2D customTexture = Game1.content.Load<Texture2D>(treeData.Texture);
            ...
            if (treeData.Texture == null || customTexture == null)
            {
                ModEntry.instance.Monitor.LogOnce($"There are issues with BiggerTree Texture for {__instance.treeId.Value}. Skipping", LogLevel.Error);
                return true;
            }
            else if (__instance.isTemporarilyInvisible || __instance.texture == null || __instance.growthStage.Value < 4)
            {
                return true;
            }
